...
function Bulong() {
    ...
    const tableData = useSelector((state) => state.bulongImport);
    const isClicked = useSelector((state) => state.click.mounted);
    const isDoneModal = useSelector((state) => state.click.isDone);
    const isEditData = useSelector((state) => state.bulongImport.getUpdateId);
        ...
        const fetchDataCongno = async () => {
            try {
                const res = await fetch(
                    "http://localhost:8080/bulongimport/blImport-datas"
                );
                const resData = await res.json();

                const dataFilterDate = resData.bulongImport.filter((data) => {
                    const createdAtData = data.createdAt.split("-")[0];
                    return tableData.date === createdAtData;
                });
                dispatch(counterActions.dataBulongImport(dataFilterDate));
            } catch {}
        };
        fetchDataCongno();
    }, [isClicked, isDoneModal, tableData]);

    const isBulongImportDB = !!bulongImportDB;

    //thêm sản phẩm vào
    const handleAddData = (e) => {
        const input = document.getElementsByTagName("input");
        console.log(tableData);
        dispatch(bulongImportActions.handleAddData(tableData));
    ...
    const handleAddFormChange = (event) => {
        ...
        dispatch(
            bulongImportActions.handleEnteredData({ newFormData: newFormData })
        );
    };
    ...
    return (
        <Fragment>
            <div className="flex justify-between items-center">
                ...
                <div className={cx("custom-select")}>
                    <select
                        onChange={(e) => {
                            dispatch(
                                bulongImportActions.handleGetDate(
                                    e.target.value
                                )
                            );
                        }}
            ...
            <h1 className="text-5xl text-center pt-10 py-10">
                Ngày Tạo{" "}
                {isBulongImportDB &&
                    bulongImportDB.length > 0 &&
                    bulongImportDB[0].createdAt.split("T").reverse()[1]}
            </h1>
            {isDoneModal && <Done />}
            <div className={cx("wrapper")}>
                <table className={cx("table")}>
                    ...
                    <tbody>
                        ...
                        {isBulongImportDB &&
                            bulongImportDB.map((contact, index) => {
                                return (
                                    <Fragment key={contact._id}>
                                        {" "}
                                        {isEditData === contact._id ? (
                                            <EditableRowBulong data={contact} />
                                        ) : (
                                            <ReadOnlyRowBulong
                                                contact={contact}
                                                index={index}
