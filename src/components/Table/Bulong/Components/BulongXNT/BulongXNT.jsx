...
import EditableRowBulongXNT from "./EditableRowBulongXNT";
import ReadOnlyRowBulongXNT from "./ReadOnlyRowBulongXNT";
...
function BulongNeed() {
    ...
    const tableData = useSelector((state) => state.bulongXNT);
    const isClicked = useSelector((state) => state.click.mounted);
    const isDoneModal = useSelector((state) => state.click.isDone);
    const isEditData = useSelector((state) => state.bulongXNT.getUpdateId);
    console.log(isEditData);
    // hooks
    const tableRef = useRef(null);
    const bulongXNTDB = useSelector((state) => state.counter.bulongXNT);

    useEffect(() => {
        const fetchDataCongno = async () => {
            try {
                const res = await fetch(
                    "http://localhost:8080/bulongxnt/blxnt-datas"
                );
                const resData = await res.json();

                const dataFilterDate = resData.BulongXNT.filter((data) => {
                    const createdAtData = data.createdAt.split("-")[0];
                    return tableData.date === createdAtData;
                });

                dispatch(counterActions.dataBulongXNT(dataFilterDate));
            } catch {}
        };
        fetchDataCongno();
    }, [isClicked, tableData, isDoneModal]);

    const isBulongExportDB = !!bulongXNTDB;
    ...
    const handleAddData = (e) => {
        ...
        dispatch(bulongXntActions.handleAddData());
    ...
    return (
        <Fragment>
            <div className="flex justify-between items-center">
                ...
                <div className={cx("custom-select")}>
                    <select
                        onChange={(e) => {
                            dispatch(
                                bulongXntActions.handleGetDate(e.target.value)
                            );
                        }}
                    >
            ...
            <h1 className="text-5xl text-center pt-10 py-10">
                Ngày Tạo{" "}
                {isBulongExportDB &&
                    bulongXNTDB.length > 0 &&
                    bulongXNTDB[0].createdAt.split("T").reverse()[1]}
            </h1>
            {isDoneModal && <Done />}
                ...
                <table className={cx("table")}>
                    ...
                    <tbody>
                        <tr className={cx("together")}>
                            ...
                            <td>
                                <Button smaller onClick={handleAddData}>
                                    Reload
                                </Button>
                            </td>
                        </tr>
                        ...
                        {isBulongExportDB &&
                            bulongXNTDB.map((contact, index) => {
                                return (
                                    <Fragment key={contact._id}>
                                        {" "}
                                        {isEditData === contact._id ? (
                                            <EditableRowBulongXNT
                                                data={contact}
                                            />
                                        ) : (
                                            <ReadOnlyRowBulongXNT
                                                contact={contact}
                                                index={index}
                                            />
                                        )}
