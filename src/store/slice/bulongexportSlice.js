...
export const bulongExportSlice = createSlice({
    name: "bulongexport",
    ...
    reducers: {
        ...
        handleEnteredData(state, action) {
            ...
            if (action.payload.newFormData.m18230) {
                state.m18230 = action.payload.newFormData.m18230;
            }
            if (action.payload.newFormData.m18260) {
                state.m18260 = action.payload.newFormData.m18260;
            }
            if (action.payload.newFormData.m18280) {
        ...
        handleAddData(state, action) {
            ...
            const postData = async () => {
                try {
                    fetch("http://localhost:8080/bulongexport/postBlexport", {
                        method: "POST",
                        body: JSON.stringify({
        ...
        handleDeleteData(state, action) {
            console.log(action.payload.bulongExportId);
            if (action.payload.bulongExportId) {
                fetch(
                    `http://localhost:8080/bulongexport/postBlexport/${action.payload.bulongExportId}`,
                    {
                        method: "DELETE",
                        body: JSON.stringify({
        ...
        handleUpdateData(state, action) {
            ...
            const RequestUpdateData = () => {
                try {
                    fetch(
                        `http://localhost:8080/bulongexport/update-blexport/${action.payload.id}`,
                        {
                            method: "PUT",
                            body: JSON.stringify({
