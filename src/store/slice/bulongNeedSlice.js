...
export const bulongNeedSlice = createSlice({
    name: "bulongNeedSlice",
    ...
    reducers: {
        ...
        handleAddData(state, action) {
            ...
            const postData = async () => {
                try {
                    fetch("http://localhost:8080/bulongneed/postBlneed", {
                        method: "POST",
                        body: JSON.stringify({
        ...
        handleDeleteData(state, action) {
            console.log(action.payload);
            if (action.payload.bulongNeedId) {
                fetch(
                    `http://localhost:8080/bulongneed/postBlneed/${action.payload.bulongNeedId}`,
                    {
                        method: "DELETE",
                        body: JSON.stringify({
        ...
        handleUpdateData(state, action) {
            ...
            const RequestUpdateData = () => {
                try {
                    fetch(
                        `http://localhost:8080/bulongneed/update-blneed/${action.payload.id}`,
                        {
                            method: "PUT",
                            body: JSON.stringify({
